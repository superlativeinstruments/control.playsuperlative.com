...
async function onConfigConnect() {
	...
	result = await configPort.read(configAddresses.MIDI_TRS_IN);
	settings.midiTrsIn = !!result;

	result = await configPort.read(configAddresses.MIDI_USB_IN);
	settings.midiUsbIn = !!result;

	result = await configPort.read(configAddresses.MIDI_USB_OUT);
	settings.midiUsbOut = !!result;
...
async function save() {
	...
	await configPort.write(configAddresses.MIDI_TRS_IN, settings.value.midiTrsIn ? 0x01 : 0x00);
	await configPort.write(configAddresses.MIDI_USB_IN, settings.value.midiUsbIn ? 0x01 : 0x00);
	await configPort.write(configAddresses.MIDI_USB_OUT, settings.value.midiUsbOut ? 0x01 : 0x00);
...
<div class="hero bg-base-300 w-full max-w-xl rounded-xl">
	<div>
		<header class="w-full bg-black/15 rounded-t-xl mb-8 p-2 px-4">
			<h1 class="text-neutral">CICADA / CONTROLS</h1>
		</header>
		...
		<div class="grid gap-4 w-screen max-w-xl p-12 pt-0">
			<div class="grid auto-rows-fr gap-4">
				<div class="form-control w-full">
					<label class="cursor-pointer label">
						<span class="label-text text-xl">MIDI TRS in</span>
						<input type="checkbox"
				...
				<div class="form-control w-full">
					<label class="cursor-pointer label">
						<span class="label-text text-xl">MIDI USB in</span>
						<input type="checkbox"
							   class="toggle toggle-lg toggle-accent"
				...
				<div class="form-control w-full">
					<label class="cursor-pointer label">
						<span class="label-text text-xl">MIDI USB out</span>
						<input type="checkbox"
							   class="toggle toggle-lg toggle-accent"
			...
			<div class="grid gap-4 form-control w-full mb-12">
				<label class="label justify-start gap-2">
					...
					<div class="tooltip" data-tip="MIDI clock is 24ppm. This means that 24 clock ticks represent a single quarter note. You can divide this with the slider below so that the CICADA LFO is triggered faster or slower than the standard quarter note">
